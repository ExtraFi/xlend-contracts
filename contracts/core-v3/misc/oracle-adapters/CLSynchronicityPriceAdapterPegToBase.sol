 ...
 * @notice based on cbETH / ETH and ETH / USD feeds.
 */
contract CLSynchronicityPriceAdapterPegToBase is ICLSynchronicityPriceAdapter {
  /**
   * @notice Price feed for (Base / Peg) pair, eg. ETH / USD
   */
  IChainlinkAggregator public immutable PEG_TO_BASE;

  /**
   * @notice Price feed for (Asset / Peg) pair, eg. cbETH / ETH
   */
  ...
  function latestAnswer() public view virtual override returns (int256) {
    (, int256 assetToPegPrice, , , ) = ASSET_TO_PEG.latestRoundData();
    (, int256 pegToBasePrice, , , ) = PEG_TO_BASE.latestRoundData();
