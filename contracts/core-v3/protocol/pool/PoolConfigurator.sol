...
contract PoolConfigurator is VersionedInitializable, IPoolConfigurator {
  ...
  /// @inheritdoc IPoolConfigurator
  function updateReserveDecimals(address asset, uint8 decimals) external override onlyPoolAdmin {
  ...
  //   DataTypes.ReserveConfigurationMap memory currentConfig = _pool.getConfiguration(asset);
  //   if (enabled) {
  //     revert(Errors.STABLE_BORROWING_NOT_ALLOWERD);
  ...
  function setReserveFactor(
    ...
    // _pool.updateReserveState(asset);
    ...
    // _pool.updateReserveIndexRate(asset);
  ...
  function setReserveTreasury(address asset, address treasury) external override onlyPoolAdmin {
    _pool.setReserveTreasury(asset, treasury);
  }
  ...
  function setReserveInterestRateData(
    ...
    DataTypes.ReserveData memory reserve = _pool.getReserveData(asset);
    IDefaultInterestRateStrategyV2(reserve.interestRateStrategyAddress).setInterestRateParams(asset, rateData);
  } 
